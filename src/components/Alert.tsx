...
export function Alert({
  ...
  return (
    <div className="pointer-events-none fixed inset-x-0 top-6 z-[100] flex justify-center px-4">
      <div
        role="alert"
        className={`alert relative overflow-hidden rounded-none border-0 border-l-[1.5px] border-t-[1.5px] border-[#2f2229] text-base-content transition-all duration-300 ${
      ...
      >
        ...
        <span
          aria-hidden
          className={`pointer-events-none absolute bottom-0 left-0 top-0 w-1.5 ${VARIANT_BAR[variant]}`}
        />
        {children}
      </div>
    </div>
